...
- (void)setSelectedIndex:(NSInteger)selectedIndex {
    if (selectedIndex < _columns && _selectedIndex != selectedIndex) {
...
- (void)loadData {
    if ([_dataSource respondsToSelector:@selector(numberOfColumnsInListView:)]) {
        ...
        for (int index = 0; index < _columns; index ++) {
            ...
            if ([_dataSource respondsToSelector:@selector(widthForColumnAtIndex: index:)]) {
                width = [_dataSource widthForColumnAtIndex:self index:index];
            }
